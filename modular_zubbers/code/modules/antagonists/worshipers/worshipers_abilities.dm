...
/datum/action/cooldown/spell/pointed/enlighten_worshiper/cast(atom/cast_on)
	...
	if (cast_on == deity)
		to_chat(owner, span_warning("You cant join your own cult!"))
		ResetCooldown()
		return FALSE
	if (isnull(convertee))
		to_chat(owner, span_warning("Only humanoids can worship you!"))
		ResetCooldown()
		return FALSE
	if (tgui_alert(convertee, "So you want to become a Worshiper of the Divine Presence?", "Become a Worshiper?", list("Join", "Do Not"), timeout = 30 SECONDS) != "Join")
		to_chat(owner, span_warning("This one doesn't want to worship you!"))
		return FALSE

	//TODO: grant the antag datum here, including reference to the deity
